...
import { newLogger, nonEmptyArr, isEmptyArray } from '@darkpay/dark-utils';
...
const createNofitParams = (count): LetterParams => ({
  fromName: 'Darkdot Notifications',
  title: `Notifications for ${getWeeklyDayDate()}`,
  subject: `You have ${count} unread notifications on Darkdot`
})

const createFeedParams = (): LetterParams => ({
  fromName: 'Darkdot Feed Updates',
  title: `Notifications for ${getWeeklyDayDate()}`,
  subject: 'You have new products in your feed on Darkdot'
})
